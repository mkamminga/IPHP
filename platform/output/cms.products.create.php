	...
	<?php if (!isset($userGuard) || $userGuard != $__view->getInjectedVar("userGuard")){$userGuard=$__view->getInjectedVar("userGuard");} ?>
        ...
        <div class="row">
            ...
            <div class="large-9 columns">    
                <h2> Nieuw product</h2>
                <div class="content">
                    <?php if (!isset($categories) || $categories != $__view->getInjectedVar("categories")){$categories=$__view->getInjectedVar("categories");}if (!isset($subCategories) || $subCategories != $__view->getInjectedVar("subCategories")){$subCategories=$__view->getInjectedVar("subCategories");}if (!isset($vatRates) || $vatRates != $__view->getInjectedVar("vatRates")){$vatRates=$__view->getInjectedVar("vatRates");}if (!isset($errors) || $errors != $__view->getInjectedVar("errors")){$errors=$__view->getInjectedVar("errors");} ?>
					<?php
	if (isset($errors)):
